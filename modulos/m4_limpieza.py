    ...
    if not df_base.empty:
        ...
        if st.button("🚀 Ejecutar Limpieza Estructural (Sanitización)", type="primary"):
            with st.spinner("Destruyendo espacios, unificando formatos y purgando duplicados..."):
                ...
                # 2. Limpieza de texto profunda (NATIVA DE PANDAS - ANTI ERRORES)
                cols_texto = df_limpio.select_dtypes(include=['object', 'string']).columns
                for col in cols_texto:
                    # Convertir a texto seguro, quitar espacios en los bordes y pasar a mayúsculas
                    df_limpio[col] = df_limpio[col].astype(str).str.strip().str.upper()
                    # Reemplazar múltiples espacios internos por uno solo usando el motor de Pandas
